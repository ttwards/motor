...
void IMU_QuaternionEKF_Update(float gx, float gy, float gz, float ax, float ay, float az,
    ...
    */
    ...
    QEKF_INS.Accel[0] =
        QEKF_INS.Accel[0] * QEKF_INS.accLPFcoef / (QEKF_INS.dt + QEKF_INS.accLPFcoef) +
        ax * QEKF_INS.dt / (QEKF_INS.dt + QEKF_INS.accLPFcoef);
    QEKF_INS.Accel[1] =
        QEKF_INS.Accel[1] * QEKF_INS.accLPFcoef / (QEKF_INS.dt + QEKF_INS.accLPFcoef) +
        ay * QEKF_INS.dt / (QEKF_INS.dt + QEKF_INS.accLPFcoef);
    QEKF_INS.Accel[2] =
        QEKF_INS.Accel[2] * QEKF_INS.accLPFcoef / (QEKF_INS.dt + QEKF_INS.accLPFcoef) +
        az * QEKF_INS.dt / (QEKF_INS.dt + QEKF_INS.accLPFcoef);

    // set z,单位化重力加速度向量
    accelInvNorm =
        invSqrt(QEKF_INS.Accel[0] * QEKF_INS.Accel[0] + QEKF_INS.Accel[1] * QEKF_INS.Accel[1] +
    ...
    // get body state
    QEKF_INS.gyro_norm =
        1.0f / invSqrt(QEKF_INS.Gyro[0] * QEKF_INS.Gyro[0] + QEKF_INS.Gyro[1] * QEKF_INS.Gyro[1] +
...
void IMU_Sensor_handle_update(CACHED_IMU_DATA_t *data) {
    if (data->recv_flag < 2) {
        return;
    }
    data->recv_flag = 0;
    IMU_QuaternionEKF_Update(data->accel[0], data->accel[1], data->accel[2], data->gyro[0],
                             data->gyro[1], data->gyro[2], data->dt);
}

void IMU_Sensor_trig_handler(const struct device *dev, const struct sensor_trigger *trigger) {
    if (trigger->type != SENSOR_TRIG_DATA_READY) {
        return;
    }
    float current_time = k_cyc_to_sec_near32(k_cycle_get_32());
    ...
    if (trigger->chan == SENSOR_CHAN_ACCEL_XYZ) {
        ...
        cachedData.dt += (cachedData.last_time_accel - current_time) / 2;
        cachedData.last_time_accel = current_time;
        cachedData.recv_flag++;

        cachedData.accel[0] = sensor_value_to_double(&accel_data[0]);
        cachedData.accel[1] = sensor_value_to_double(&accel_data[1]);
        cachedData.accel[2] = sensor_value_to_double(&accel_data[2]);
        ...
        cachedData.dt += (cachedData.last_time_gyro - current_time) / 2;
        cachedData.last_time_gyro = current_time;
        cachedData.recv_flag++;

        cachedData.accel[0] = sensor_value_to_double(&gyro_data[0]);
        cachedData.accel[1] = sensor_value_to_double(&gyro_data[1]);
        cachedData.accel[2] = sensor_value_to_double(&gyro_data[2]);
    }

    IMU_Sensor_handle_update(&cachedData);
}
...
static void IMU_Sensor_trig_init(const struct device *accel_dev, const struct device *gyro_dev) {
    ...
    cachedData.last_time_gyro  = current_time;
    cachedData.last_time_accel = current_time;
    cachedData.recv_flag       = 0;
    sensor_trigger_set(accel_dev, &trig, &IMU_Sensor_trig_handler);
    sensor_trigger_set(gyro_dev, &trig, &IMU_Sensor_trig_handler);
}