...
static void single_pid_calc(const struct device *pid_dev) {
    ...
    if (!float_equal(kd, 0)) {
        if (pid_data->mode == NORMAL_PID)
            pid_data->err_derivate = kd * err / deltaT;
        else if (pid_data->mode == MIT_PID)
            pid_data->err_derivate =
                kd * (*(pid_data->mit_v_ref) - *(pid_data->mit_v_curr)) / deltaT;
    }
...
static void single_pid_mit_vert(const struct device *pid_dev, float *curr, float *ref) {
    struct pid_single_driver_data *pid_data = pid_dev->data;
    pid_data->mit_v_curr                    = curr;
    pid_data->mit_v_ref                     = ref;
    pid_data->mode                          = MIT_PID;
    return;
}
...
#define PID_CONFIG_DEFINE(inst)                                                                    \
    static const struct pid_single_driver_config pid_single_driver_config_##inst =                 \
        PID_SINGLE_DT_DRIVER_CONFIG_GET(DT_DRV_INST(inst));

#define PID_DATA_DEFINE(inst)                                                                      \
    static struct pid_single_driver_data pid_single_driver_data_##inst = {.err_integral = 0,       \
                                                                          .err_derivate = 0,       \
                                                                          .curr         = NULL,    \
                                                                          .ref          = NULL,    \
                                                                          .output       = NULL,    \
...
#define PID_INST(inst)                                                                             \
    ...
    PID_DEVICE_DT_DEFINE(DT_DRV_INST(inst), NULL, NULL, &pid_single_driver_data_##inst,            \
